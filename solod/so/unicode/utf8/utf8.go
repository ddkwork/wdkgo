...
//
// Based on the [unicode/utf8] package.
//
...
func decodeRuneSlow(p []byte) (rune, int) {
	n := len(p)
	if n < 1 {
		return RuneError, 0
	}
...
func decodeRuneInStringSlow(s string) (rune, int) {
	...
	if sz <= 2 { // <= instead of == to help the compiler eliminate some bounds checks
		r := rune(s0&mask2)<<6 | rune(s1&maskx)
		return r, 2
	}
	s2 := s[2]
	if s2 < locb || hicb < s2 {
	...
	if sz <= 3 {
		r := rune(s0&mask3)<<12 | rune(s1&maskx)<<6 | rune(s2&maskx)
		return r, 3
	}
	s3 := s[3]
	if s3 < locb || hicb < s3 {
	...
	r := rune(s0&mask4)<<18 | rune(s1&maskx)<<12 | rune(s2&maskx)<<6 | rune(s3&maskx)
	return r, 4
}
...
func encodeRuneNonASCII(p []byte, r rune) int {
	...
	if i <= rune2Max {
		...
		p[2] = tx | (byte(r) & maskx)
		return 3
	} else if i > rune3Max && i <= uint32(MaxRune) {
...
func wordString(s string) uintptr {
	if ptrSize == 4 {
		return uintptr(s[0]) | uintptr(s[1])<<8 | uintptr(s[2])<<16 | uintptr(s[3])<<24
	}
	return uintptr(uint64(s[0]) | uint64(s[1])<<8 | uint64(s[2])<<16 | uint64(s[3])<<24 | uint64(s[4])<<32 | uint64(s[5])<<40 | uint64(s[6])<<48 | uint64(s[7])<<56)
...
func Valid(p []byte) bool {
	...
	for len(p) > 0 {
		...
		if p0 < RuneSelf {
			...
			if len(p) > ptrSize && (wordSlice(p)&hiBits) == 0 {
				p = p[ptrSize:]
				if len(p) > 2*ptrSize && ((wordSlice(p)|wordSlice(p[ptrSize:]))&hiBits) == 0 {
					p = p[2*ptrSize:]
					for len(p) > 4*ptrSize && (((wordSlice(p)|wordSlice(p[ptrSize:]))|(wordSlice(p[2*ptrSize:])|wordSlice(p[3*ptrSize:])))&hiBits) == 0 {
						p = p[4*ptrSize:]
					}
				}
			}
...
func ValidString(s string) bool {
	for len(s) > 0 {
		...
		if s0 < RuneSelf {
			...
			// Note: using > instead of >= here is intentional. That avoids
			// needing pointing-past-the-end fixup on the slice operations.
			if len(s) > ptrSize && (wordString(s)&hiBits) == 0 {
				s = s[ptrSize:]
				if len(s) > 2*ptrSize && ((wordString(s)|wordString(s[ptrSize:]))&hiBits) == 0 {
					s = s[2*ptrSize:]
					for len(s) > 4*ptrSize && (((wordString(s)|wordString(s[ptrSize:]))|(wordString(s[2*ptrSize:])|wordString(s[3*ptrSize:])))&hiBits) == 0 {
						s = s[4*ptrSize:]
					}
				}
			}
