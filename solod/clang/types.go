...
type CType struct {
	...
	// ArrayOfPtrToArray: true when the type is an array of pointers to arrays.
	// e.g. [511]*[512]EptEntry -> EptEntry (*name[511])[512]
	ArrayOfPtrToArray bool
	InnerDims         string // dims of the inner array the pointer points to
}

// Decl formats a C declaration: "int name[3]".
func (t CType) Decl(name string) string {
	if t.ArrayOfPtrToArray {
		return t.Base + " (*" + name + t.Dims + ")" + t.InnerDims
	}
	if t.PtrToArray {
...
func (t CType) IsArray() bool {
	return t.Dims != "" && !t.PtrToArray && !t.ArrayOfPtrToArray
}
...
func (g *Generator) mapCType(node ast.Node, typ types.Type) CType {
	...
	// Check for array of pointers to arrays: [N]*[M]T
	if arr, ok := types.Unalias(typ).(*types.Array); ok {
		if ptr, ok := types.Unalias(arr.Elem()).(*types.Pointer); ok {
			if innerArr, ok := types.Unalias(ptr.Elem()).(*types.Array); ok {
				return CType{
					Base:              g.mapType(node, innerArr.Elem()),
					Dims:              fmt.Sprintf("[%d]", arr.Len()),
					ArrayOfPtrToArray: true,
					InnerDims:         arrayDims(innerArr),
				}
			}
		}
	}
...
func (g *Generator) symbolName(name string) string {
	if info, ok := g.getExtern("", name); ok {
		if info.name != "" {
			return info.name
		}
		return name
...
func (g *Generator) funcCName(decl *ast.FuncDecl) string {
	if info, ok := g.getExtern("", externFuncKey(decl)); ok {
		if info.name != "" {
			return info.name
		}
