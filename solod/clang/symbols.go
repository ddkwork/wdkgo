...
// Generator is extended with externTypes to track //so:extern type symbols.
type symbol struct {
	kind     symbolKind
	exported bool
...
// externTypeSymbol holds info about a //so:extern type for typedef generation.
type externTypeSymbol struct {
	name     string
	typeSpec *ast.TypeSpec
	info     externInfo
}
...
func (g *Generator) collectSymbols() {
	...
	for _, file := range g.pkg.Syntax {
		for _, decl := range file.Decls {
			switch d := decl.(type) {
			case *ast.GenDecl:
				if found, _ := parseExternDirective(d.Doc); found && d.Tok != token.TYPE {
					continue
				}
				switch d.Tok {
				case token.TYPE:
					for _, spec := range d.Specs {
						...
						if g.hasExtern("", ts.Name.Name) {
							info, _ := g.getExtern("", ts.Name.Name)
							g.externTypeSymbols = append(g.externTypeSymbols, externTypeSymbol{
								name:     ts.Name.Name,
								typeSpec: ts,
								info:     info,
							})
			...
			case *ast.FuncDecl:
				...
				isExtern := g.hasExtern("", externFuncKey(d))
				if d.Body == nil && !isExtern {
					continue
				}
				if isExtern {
					g.symbols = append(g.symbols, symbol{
						kind:     symbolExternFunc,
