...
func (g *Generator) emitMapIndexAssign(node ast.Node, idx *ast.IndexExpr, rhs ast.Expr) {
	...
	fmt.Fprintf(w, "%s{\n", g.indent())
	fmt.Fprintf(w, "%s\t%s _so_map_assign_val;\n", g.indent(), valType)
	fmt.Fprintf(w, "%s\tmemset(&_so_map_assign_val, 0, sizeof(_so_map_assign_val));\n", g.indent())
	fmt.Fprintf(w, "%s\t_so_map_assign_val = ", g.indent())
	g.emitExprAsType(node, rhs, mapType.Elem())
	fmt.Fprintf(w, ";\n")
	fmt.Fprintf(w, "%s\tso_map_set(%s, %s, ", g.indent(), keyType, valType)
	g.emitExpr(idx.X)
	fmt.Fprintf(w, ", ")
	g.emitExpr(idx.Index)
	fmt.Fprintf(w, ", _so_map_assign_val);\n")
	fmt.Fprintf(w, "%s}\n", g.indent())
}
