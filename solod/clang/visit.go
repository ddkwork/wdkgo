...
func (g *Generator) emitConstSpec(spec *ast.ValueSpec) {
	...
	for i, name := range spec.Names {
		...
		if g.state.indent == 0 {
			...
			val := obj.Val()
			fmt.Fprintf(w, "%s#define %s ", g.indent(), constName)
			g.emitConstDefineValue(val, cType)
			fmt.Fprintf(w, "\n")
			continue
		}
...
func (g *Generator) emitConstDefineValue(val constant.Value, cType string) {
	switch val.Kind() {
	case constant.Int:
		if v, ok := constant.Int64Val(val); ok {
			g.emitDefineCast(v, cType)
		} else if v, ok := constant.Uint64Val(val); ok {
			switch cType {
			case "uint64_t", "uintptr_t":
				fmt.Fprintf(g.state.writer, "%dULL", v)
			default:
				fmt.Fprintf(g.state.writer, "((%s)%dULL)", cType, v)
			}
		} else {
			fmt.Fprintf(g.state.writer, "((%s)%s)", cType, val.ExactString())
		}
	case constant.Bool:
		if constant.BoolVal(val) {
			fmt.Fprintf(g.state.writer, "true")
		} else {
			fmt.Fprintf(g.state.writer, "false")
		}
	case constant.String:
		s := constant.StringVal(val)
		fmt.Fprintf(g.state.writer, "so_str(\"%s\")", s)
	default:
		fmt.Fprintf(g.state.writer, "((%s)%s)", cType, val.ExactString())
	}
}

func (g *Generator) emitConstValDefine(node ast.Node, name *ast.Ident, cType string) {
	obj := g.types.Defs[name].(*types.Const)
	val := obj.Val()
	switch val.Kind() {
	case constant.Int:
...
func (g *Generator) emitExprDefine(expr ast.Expr, cType string) {
	g.emitExprDefineWithCast(expr, cType)
}

func (g *Generator) emitDefineCast(v int64, cType string) {
	switch cType {
	case "uint64_t", "uintptr_t":
		fmt.Fprintf(g.state.writer, "%dULL", v)
	case "int64_t", "so_int":
		fmt.Fprintf(g.state.writer, "%dLL", v)
	case "uint32_t":
		fmt.Fprintf(g.state.writer, "%dU", v)
	case "int32_t":
		fmt.Fprintf(g.state.writer, "%d", v)
	case "uint16_t":
		fmt.Fprintf(g.state.writer, "((uint16_t)%d)", v)
	case "uint8_t":
		fmt.Fprintf(g.state.writer, "((uint8_t)%d)", v)
	case "bool":
		if v != 0 {
			fmt.Fprintf(g.state.writer, "true")
		} else {
	...
	default:
		fmt.Fprintf(g.state.writer, "((%s)%d)", cType, v)
	}
}

func (g *Generator) emitExprDefineWithCast(expr ast.Expr, cType string) {
	switch e := expr.(type) {
	case *ast.BinaryExpr:
		fmt.Fprintf(g.state.writer, "(")
		g.emitExprDefineWithCast(e.X, cType)
		switch e.Op {
		case token.ADD:
			fmt.Fprintf(g.state.writer, " + ")
		case token.SUB:
		...
		case token.SHR:
			fmt.Fprintf(g.state.writer, " >> ")
		default:
			fmt.Fprintf(g.state.writer, " %s ", e.Op)
		}
		g.emitExprDefineWithCast(e.Y, cType)
		fmt.Fprintf(g.state.writer, ")")
	case *ast.ParenExpr:
		fmt.Fprintf(g.state.writer, "(")
		g.emitExprDefineWithCast(e.X, cType)
		fmt.Fprintf(g.state.writer, ")")
	case *ast.Ident:
		if def, ok := g.types.Defs[e]; ok {
			if _, ok := def.(*types.Const); ok {
				fmt.Fprintf(g.state.writer, "%s", g.symbolName(e.Name))
				return
			}
		}
		fmt.Fprintf(g.state.writer, "%s", g.symbolName(e.Name))
...
func (g *Generator) emitVarSpec(spec *ast.ValueSpec) {
	...
	for i, name := range spec.Names {
		...
		if len(spec.Values) > i {
			if g.state.indent == 0 && isGlobalErrorVar(spec, typ, spec.Values[i]) {
				msg := g.extractErrorMsg(spec.Values[i])
				if msg != "" {
					fmt.Fprintf(w, "SO_GLOBAL_ERROR(%s, \"%s\");\n", cName, msg)
					continue
				}
			}
			if g.state.indent == 0 && !isConstInitializer(spec.Values[i]) {
				zeroVal := g.zeroValue(spec, typ)
				fmt.Fprintf(w, "%s%s%s = %s;\n", g.indent(), specifier, ct.Decl(cName), zeroVal)
				g.deferredInits = append(g.deferredInits, deferredInit{
					cName: cName,
					cType: ct.Base,
					expr:  spec.Values[i],
					typ:   typ,
...
func isGlobalErrorVar(spec *ast.ValueSpec, typ types.Type, expr ast.Expr) bool {
	iface, ok := typ.(*types.Named)
	if !ok {
		return false
	}
	...
	return fn.Name == "fmtError" || fn.Name == "errors_New"
}

func isConstInitializer(expr ast.Expr) bool {
	switch e := expr.(type) {
	case *ast.BasicLit:
		return true
	case *ast.Ident:
		return true
	case *ast.UnaryExpr:
		return isConstInitializer(e.X)
	case *ast.BinaryExpr:
		return isConstInitializer(e.X) && isConstInitializer(e.Y)
	case *ast.ParenExpr:
		return isConstInitializer(e.X)
...
func (g *Generator) extractErrorMsg(expr ast.Expr) string {
	call, ok := expr.(*ast.CallExpr)
	if !ok {
		return ""
	}
...
func (g *Generator) emitReturnExpr(stmt *ast.ReturnStmt) {
	...
	if g.state.funcSig != nil && g.state.funcSig.Results().Len() > 0 {
