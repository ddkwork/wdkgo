...
func (g *Generator) emitBinaryExpr(n *ast.BinaryExpr) {
	...
	// Struct comparisons: C does not support == or != on structs.
	// Emit memcmp-based comparison instead.
	if n.Op == token.EQL || n.Op == token.NEQ {
		if _, ok := g.types.TypeOf(n.X).Underlying().(*types.Struct); ok {
			cType := g.mapType(n, g.types.TypeOf(n.X))
			if n.Op == token.EQL {
				fmt.Fprintf(w, "(memcmp(&")
			} else {
				fmt.Fprintf(w, "!memcmp(&")
			}
			g.emitExpr(n.X)
...
func (g *Generator) emitCallExpr(n *ast.CallExpr) {
	...
	if tv, ok := g.types.Types[n.Fun]; ok && tv.IsType() {
		...
		srcType := g.types.TypeOf(n.Args[0])
		if types.Identical(srcType, tv.Type) || types.Identical(srcType.Underlying(), tv.Type.Underlying()) {
...
func (g *Generator) emitSelectorExpr(n *ast.SelectorExpr) {
	if ident, ok := n.X.(*ast.Ident); ok {
		if pkgName, ok := g.types.Uses[ident].(*types.PkgName); ok {
			...
			if pkgName.Name() == "wdk" {
				fmt.Fprintf(g.state.writer, "%s", n.Sel.Name)
			} else {
...
func (g *Generator) tryEmitUTF16Encode(n *ast.CallExpr) bool {
	sel, ok := n.Fun.(*ast.SelectorExpr)
	if !ok || sel.Sel.Name != "Encode" {
		return false
	}
	if ident, ok := sel.X.(*ast.Ident); !ok || ident.Name != "utf16" {
		return false
	}
	if len(n.Args) != 1 {
		return false
	...
	if call, ok := arg.(*ast.CallExpr); ok {
		if len(call.Args) == 1 {
			if blit, ok := call.Args[0].(*ast.BasicLit); ok && blit.Kind == token.STRING {
				tv, ok := g.types.Types[call.Fun]
				if ok && tv.IsType() {
					if sl, ok := tv.Type.Underlying().(*types.Slice); ok {
						if elem, ok := sl.Elem().(*types.Basic); ok && elem.Kind() == types.Int32 {
							strLit = blit.Value
						}
					}
				}
	...
	} else if blit, ok := arg.(*ast.BasicLit); ok && blit.Kind == token.STRING {
		strLit = blit.Value
	}
	if strLit == "" {
		return false
	}
	s, err := unquoteString(strLit)
	if err != nil {
		return false
	}
	w := g.state.writer
	if g.isKernelMode() {
		fmt.Fprintf(w, "((so_Slice){(void*)L\"")
		for _, r := range s {
			fmt.Fprintf(w, "%s", escapeWideChar(r))
		}
		fmt.Fprintf(w, "\", %d, %d})", len(s)+1, len(s)+1)
	} else {
		fmt.Fprintf(w, "L\"")
		for _, r := range s {
			fmt.Fprintf(w, "%s", escapeWideChar(r))
