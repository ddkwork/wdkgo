...
func (g *Generator) emitBuiltin(call *ast.CallExpr, ident *ast.Ident, bi *types.Builtin) bool {
	...
	case "clear":
		...
		return true
	case "close", "complex", "imag", "real", "recover":
		g.fail(call, "%s() is not supported", bi.Name())
		return true
	case "delete":
		g.emitDeleteCall(call)
		return true
	case "copy":
		g.emitCopyCall(call)
		return true
...
func (g *Generator) emitDeleteCall(call *ast.CallExpr) {
	w := g.state.writer
	if len(call.Args) < 2 {
		fmt.Fprintf(w, "0")
		return
	}
	mapType := g.types.TypeOf(call.Args[0]).Underlying()
	if m, ok := mapType.(*types.Map); ok {
		keyType := g.mapType(call, m.Key())
		fmt.Fprintf(w, "so_map_remove(%s, ", keyType)
		g.emitExpr(call.Args[0])
		fmt.Fprintf(w, ", ")
		g.emitExpr(call.Args[1])
		fmt.Fprintf(w, ")")
...
func (g *Generator) emitMakeCall(call *ast.CallExpr) {
	...
	typeInfo, ok := g.types.Types[call.Args[0]]
	if !ok || typeInfo.Type == nil {
		fmt.Fprintf(w, "0")
		return
	}
	typ := typeInfo.Type.Underlying()
	if typ == nil {
		fmt.Fprintf(w, "0")
		return
	}
	...
	case *types.Slice:
		...
		fmt.Fprintf(w, "so_make_slice_impl(sizeof(%s), ", elemType)
		g.emitExpr(call.Args[1])
		fmt.Fprintf(w, ", ")
		if len(call.Args) >= 3 {
	...
	case *types.Map:
		...
		fmt.Fprintf(w, "so_make_map_impl(sizeof(%s), sizeof(%s), ", keyType, valType)
		if len(call.Args) >= 2 {
			g.emitExpr(call.Args[1])
		} else {
			fmt.Fprintf(w, "0")
